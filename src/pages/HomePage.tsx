  ...
  useEffect(() => {
    ...
    async function loadStats() {
      try {
        ...
        const latest = completedPlans[0] ?? null;

        setLastPlan(latest);
        setTotal(completedPlans.length);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
      ...
      <div className="relative z-10 page-inner">
        ...
        <header className="flex items-center justify-between pt-10 pb-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-medium tracking-widest uppercase text-charcoal-600">
                Sthairya Mini
              </p>

              <p className="text-[14px] font-medium text-charcoal-200 mt-0.5">
                {greeting()}
              </p>
            </div>
          </div>
        ...
        {!loading && (
          ...
          >
            <StatMini
              value={String(totalRescued)}
              label="Habits rescued"
            />

            <StatMini
              value={
                lastPlan
                  ? `${lastPlan.duration_minutes}m`
                  : '—'
              }
              label="Last session"
            />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[12px] text-charcoal-600">
                    {lastPlan.duration_minutes} min ·{' '}
                    {lastPlan.steps.length} steps
                  </p>
        ...
        >
          <div className="flex items-start gap-3 border border-charcoal-800/70 rounded-2xl px-4 py-4 bg-charcoal-900/20">
            ...
            <p className="text-[12px] text-charcoal-500 leading-relaxed">
              ...
              A 2-minute session keeps the chain alive better than a skipped
              day.
            </p>
          </div>
        </div>
...
function StatMini({
  ...
  return (
    <div className="glass-card p-4">
      <p className="text-[26px] font-semibold gold-text tracking-tightest leading-none mb-1.5">
        {value}
      </p>
